var D3graph = (function() {
	...
	var sessionChanged = false,
		getSessChanged = function() {
			return sessionChanged;
		},
		setSessChanged = function() {
			sessionChanged = false;
		};
	
	var range1dates = [],
		getRange1dates = function() {
			return range1dates;
		},
		range2dates = [],
		getRange2dates = function() {
			return range2dates;
		};
	...
	*/
	...
	function submitNotes() {
		// if (notesSubmitted == false) {
			...
			$.ajax({
				...
				error: function() {
					window.alert('Error saving clinician notes.');
				}
			});
		// }
	}
	...
	var deleteNote = function(id) {
		...
		if (delConf == true) {
			for (var i = 0; i < clinicianNotes.notes.length; i++) {
				if (clinicianNotes.notes[i].noteID == id) 
	...
	function createClinicianNotes() {
		$('#svgParent').append('<div title="Clinician note" class="clinicianNote cnStart" id="cn-' + patient + '_' + clinician + '_' + clinicianNotes.nextID + '">&nbsp;</div>');
		$('#cn-' + patient + '_' + clinician + '_' + clinicianNotes.nextID).draggable({
			axis: 'x',
			...
			stop: function() {
				...
				if (left >= 0 && left <= mgFocus.width) {
					...
					$('#cn-' + patient + '_' + clinician + '_' + clinicianNotes.nextID).remove();	/* ALERT: This is causing a problem */
					focus.append('image')
						.attr({
							...
							'y': '216',
							'class': 'clinicianNote',
							'id': 'cn-' + clinicianNotes.notes[currNote].noteID
						})
						.call(dragCnote);
					
					var dateString = getDateString(clinicianNotes.notes[currNote].Date);
					
					$('#svgParent').append('<div class="noteInline" id="ni-' + clinicianNotes.notes[currNote].noteID + '" style="left: ' + (getDateX(mgFocus.x, clinicianNotes.notes[currNote]) + 3) + 'px;">\
						<img src="images/note_box_arrow.png" width="13" height="18" class="arrow">\
						<h5>' + clinicianNotes.notes[currNote].clinicianID + '</h5>\
						<h6>' + dateString + '</h6>\
						<form>\
							<fieldset>\
								<textarea id="note-' + clinicianNotes.notes[currNote].noteID + '">' + clinicianNotes.notes[currNote].data + '</textarea>\
							</fieldset>\
						</form>\
						<a href="#" title="Close" class="img btnCloseCI" onclick="D3graph.closeNotes(); return false;"><span>X</span></a>\
						<a href="#" title="Delete" class="img btnCloseDelete" onclick="D3graph.deleteNote(\'' + clinicianNotes.notes[currNote].noteID + '\'); return false;"><span>X</span></a>\
					</div>');
					...
					$('#cn-' + id).remove();	/* ALERT: This is causing a problem */
					createClinicianNotes();
				}
			}
		});
	...
	function plotNotes(graph) {
		if (notesCreated == false) { /* ALERT: Working here. */
			notesCreated = true;
			createClinicianNotes();
		} else {
			closeNotes();
		...
		for (var i = 0; i < clinicianNotes.notes.length; i++) {
			...
			graph.insert('image', '#fml')
				.attr({
					...
					'y': '216',
					'class': 'clinicianNote',
					'id': 'cn-' + clinicianNotes.notes[i].noteID
				})
				.call(dragCnote);
			
				var dateString = getDateString(clinicianNotes.notes[i].Date);
				
				$('#svgParent').append('<div class="noteInline" id="ni-' + clinicianNotes.notes[i].noteID + '" style="display: none; left: ' + (getDateX(mgFocus.x, clinicianNotes.notes[i]) + 1) + 'px;">\
					<img src="images/note_box_arrow.png" width="13" height="18" class="arrow">\
					<h5>' + clinicianNotes.notes[i].clinicianID + '</h5>\
					<h6>' + dateString + '</h6>\
					<form>\
						<fieldset>\
							<textarea id="note-' + clinicianNotes.notes[i].noteID + '">' + clinicianNotes.notes[i].data + '</textarea>\
						</fieldset>\
					</form>\
					<a href="#" title="Close" class="img btnCloseCI" onclick="D3graph.closeNotes(); return false;"><span>X</span></a>\
					<a href="#" title="Delete" class="img btnCloseDelete" onclick="D3graph.deleteNote(\'' + clinicianNotes.notes[i].noteID + '\'); return false;"><span>X</span></a>\
				</div>');
		
				$('#cn-' + clinicianNotes.notes[i].noteID).click(function() {
					closeNotes();
	...
	function calcIntegrals(parent, grphBrush, arr, sessionLoad) {
		...
		if (grphBrush != null) {
			...
			if (sessionLoad == true) {
				if (parent == 'svgRange1') 
					firstDate = range1dates[0];
				else 
					firstDate = range2dates[0];
			} else 
				firstDate = grphBrush.extent()[0];
			// ...the newest date is the brush’s right date value
			if (sessionLoad == true) {
				if (parent == 'svgRange1') 
					lastDate = range1dates[1];
				else 
					lastDate = range2dates[1];
			} else 
		...
		for (var i = 0; i < arr.length; i++) {
			...
			for (var j = 0; j < initialData.length; j++) {
				...
				if (initialData[j].id == id) {
					...
					// console.log(initialData[j]); // ALERT: New issue just popped up here
	...
	function setIntegrals(graph, savedSession) {
		...
		switch (gph) {
			case 'focus' : 
				...
				break;
			case 'range1Graph' : 
				calcIntegrals('svgRange1', mgFocus.brush1, integralsRange1, savedSession);
				break;
			case 'range2Graph' : 
				calcIntegrals('svgRange2', mgFocus.brush2, integralsRange2, savedSession);
				break;
		}
	}
	...
	function plotData(graph) {
		...
		if (graph == focus) {
			plotComments(focus);
			/* ALERT: Plot the clinician notes here? */
			plotNotes(focus);
		}
		setIntegrals(graph);
	}
	...
	function createFocusBrush() {
		...
		var extent = d3.svg.brush()
			...
			.on('brush', function() {
				...
				//console.log ("here is mgFocus.line", focus.select('#focus').selectAll('path').attr('d', mgFocus.line));
				focus.selectAll('path').filter('.dataPath').attr('d', mgFocus.line);
				focus.selectAll('path').filter('.dataMa').attr('d', mgFocus.line);
	...
	function setRangeDates(id, rangeArr) {
		for (var i = 0; i < rangeArr.length; i++) {
			rangeArr[i] = new Date(rangeArr[i]);
		}
		if (id == 1) 
			range1dates = rangeArr;
		else 
			range2dates = rangeArr;
	}
	
	function savedSessionRange(id) {
		var range = (id == 1) ? range1 : range2;
		...
		if (id == 1) 
			rgRange.x.domain(range1dates);
		else 
			rgRange.x.domain(range2dates);
		...
		var extent = d3.svg.brush()
			...
			.on('brush', function() {	// 'brushstart' is what I'm going to want to listen for
				...
				if ($('#r-' + id + ' rect.extent').attr('width') > 0) {
					...
					// Select all of the target range's graphed lines
					range.selectAll('path').filter('.dataPath').attr('d', rgRange.line);
					range.selectAll('path').filter('.dataMa').attr('d', rgRange.line);
	...
	function updateData() {
		...
		*/
		...
		for (var i = 0; i < initialData.length; i++) {
			...
			if (initialData[i].id != 'comment' && initialData[i].id != 'notes' && initialData[i].id != 'sessions') {
			...
			// If the current data collection is 'comment' and its length is less than 1...
			} else if (initialData[i].id == 'notes'/* && notesCreated == false*/) {
				clinicianNotes.patientID = patient;
				clinicianNotes.nextID = initialData[i].results.nextID;
				// For each comment entry...
				for (var m = 0; m < initialData[i].results.notes.length; m++) {
					// ...create a matching object in the comments array containing the date and data
	...
	function addFocusContext() {
		...
		focus.append('rect')
			.attr({
				'width': '18',
				'height': '220',//296 as the original but covered clinican notes
				'x': '1043',
				'y': '-2',
	...
	function addSwatches() {	/* ALERT: Big problem in here regarding the swatches. */
		...
		if (data.length > 0) {
			...
			$('#cfgGraphs').append('<div class="swatchColumns" id="scs"></div>');
	...
	function foldInSessSettings(sessSettings) {
		for (var i = 0; i < sessSettings.length; i++) {
			for (var j = 0; j < initialData.length; j++) {
				if (initialData[j].id == sessSettings[i].id) {
					initialData[j].colour = sessSettings[i].colour;
				}
			}
		}
	}
	...
	function setup() {
		...
		for (var i = 0; i < initialData.length; i++) {
			if (initialData[i].id == 'sessions') {
				...
				// console.log(initialData[i].info.current.name);
				
				if (initialData[i].info.current.name != 'Create New') {
					defaultSession = false;
					// settings = initialData[i].info.current.data.settings;	ALERT: Settings now being successfully created.
					/* 
						ALERT: I'm going to need to fold the content inside data.settings inside initialData here.
					 */
					// console.log(settings);
					foldInSessSettings(initialData[i].info.current.data.settings);
				} else 
					defaultSession = true;
			}
		}
		...
		*/
		...
		if (range1dates.length > 1) {
			savedSessionRange(1);
		}
		if (range2dates.length > 1) {
			savedSessionRange(2);
		}
	...
	return {
		getRange1dates: getRange1dates,
		getRange2dates: getRange2dates,
		getSettings: getSettings,
		getSessChanged: getSessChanged,
		setSessChanged: setSessChanged,
